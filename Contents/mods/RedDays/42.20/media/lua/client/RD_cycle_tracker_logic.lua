...
function RD_CycleTrackerLogic.RegisterNewJournal(player, idSubstring)
    ...
    if journal then
        ...
        local idPageData = RD_CycleTrackerText.getBackPage(idSubstring)
...
function RD_CycleTrackerLogic.getDataCodes(cycle)
    local stat = RD_CycleManager.getPhaseStatus(cycle) -- returns: phase, time_remaining, percent_complete
